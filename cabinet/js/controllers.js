...
function LoginCtrl($rootScope, $scope, $state, $http, $timeout, authenticate, API_PUBLIC_LOGIN_URL, localStorageService, notify) {
    $scope.working = false;
    $scope.wrong = false;

    $scope.email='';
    $scope.pwd='';
    $scope.pwdPattern='^([0-9A-Za-z_!@#$%^&.]{3,20})$';
    ...
    $scope.getEmailErrMsg = function() {
        return "Введите корректный email";
    };

    $scope.getPwdErrMsg = function() {
        return "Пароль должен быть длиной не менее 3 символов и может состоять только из латинских букв, цифр, символов (!@#$%) и знака подчеркивания";
    };

    var validateCredentials = function() {
        var pattEmail = new RegExp($scope.emailPattern);
        var pattPwd = new RegExp($scope.pwdPattern);
        if(!pattEmail.test($scope.email)) {
            return { success: false, error: "Введите корректный email" };
        }
        // if(!pattPwd.test($scope.pwd)) {
        //     return { success: false, error: "Пароль должен быть длиной не менее 3 символов и может состоять только из латинских букв, цифр, символов (!@#$%) и знака подчеркивания" };
        // }
    ...
    $scope.loginme = function () {
        ...
        $http.post(
            API_PUBLIC_LOGIN_URL,
            dataObj,
            { headers: {'Content-Type': 'application/json'} }
        ).then(function(result) {
            $scope.working = false;
            $scope.wrong = false;
            localStorageService.set('apptoken', result.data.token);
            $http.defaults.headers.common.Authorization = 'Bearer ' + result.data.token;
            $rootScope.user = {
                customerUid: result.data.customerUid,
...
function RegisterCtrl($rootScope, $scope, $state, $http, localStorageService, authenticate, BASE_PATH, notify) {
    $scope.working = false;
    $scope.wrong = false;
    $scope.error = null;
...
function ForgotPwdCtrl($scope, $http, notify) {
    $scope.email = "";
    $scope.recallPwd = function() {

        $scope.doingResolve = true;
        $http.post(
            '/public_api/recallpwd',
            { email:$scope.email },
            { headers: {'Content-Type': 'application/json'} }
        ).then(function(res) {
            $scope.working = false;
            ...
            notify({
                message: 'Ссылка для восстановления пароля отправлена по адресу: ' + $scope.email,
                position: 'center',
                duration: '5000'
            });
